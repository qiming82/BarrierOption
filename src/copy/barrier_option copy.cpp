...
double BarrierOption::price() const {
    ...
    if (pricingMethod == PricingMethod::PDE)
        {
            std::cout<< "using PDE to solve price ..."<< std::endl;
            return priceADI();
        } // isolate PDE solver
    else{
...
double BarrierOption::priceADI() const {
    // Grid parameters
    double S_max = 1.25 * B; // 180 for B=120
...
//    double theta = 0.5; // Douglas-Rachford parameter
    ...
    for (int i = 0; i <= Ns; ++i) {
        for (int j = 0; j <= Nv; ++j) {
            int k = i * (Nv + 1) + j;
            if (S[i] < B) {
                V(k) = optionType == OptionType::Call ? std::max(S[i] - K, 0.0) : std::max(K - S[i], 0.0);
            } else {
                V(k) = 0.0;
            }
        }
...
//            std::cout<< S[0]<<", "<<S[1]<<", "<<S[Ns]<<std::endl;
            ...
            for (int i = 1; i < Ns; ++i) {
                if (barrierType == BarrierType::UpAndOut && S[i] >= B + 1e-8) {
                    int k = i * (Nv + 1) + j;
                    V_star(k) = 0.0;
                    continue;
                }
...
//                double Ls = -(a * (V(k + Nv + 1) - 2 * V(k) + V(k - Nv - 1)) + 
//                              d * (V(k + Nv + 1) - V(k - Nv - 1)));
//                double Lv = -(c * (V(k + 1) - 2 * V(k) + V(k - 1)) + 
//                              e * (V(k + 1) - V(k - 1)));
//                double Lsv = -(b_coeff * (V(k + Nv + 2) - V(k + Nv) - V(k - Nv) + V(k - Nv - 2)));
//                rhs_S(i - 1) = V(k) - (1.0 - theta) * dt * (Ls + Lv + Lsv - r * V(k));
            }
...
//                for (int i = 0; i <= Ns; ++i) {
//                    int k = i * (Nv + 1) + j;
//                   std::cout << "(i,j) " <<i<<", "<<j<<", "<< A_S.coeff(i,j)<<std::endl;
//                }
//            }
...
//            A_S.coeffRef(0,0) = 1.0 + 0.5* dt*(2.0*aa + r);
//            A_S.coeffRef(0,1) = -0.5 * dt * (aa + bb);
        ...
        for (int i = 1; i < Ns; ++i) {
            if (barrierType == BarrierType::UpAndOut && S[i] >= B) {
                for (int j = 0; j <= Nv; ++j) {
                    V(i * (Nv + 1) + j) = 0.0;
                }
                continue;
            ...
            for (int j = 1; j < Nv; ++j) {
                ...
                triplets.push_back(Eigen::Triplet<double>(j - 1, j - 1, 1.0 + 0.5*dt * (2.0*cc + r)));
                triplets.push_back(Eigen::Triplet<double>(j - 1, j, -0.5* dt * (cc + dd)));
                triplets.push_back(Eigen::Triplet<double>(j - 1, j - 2, -0.5* dt * (cc - dd)));

                // Explicit terms rhs
                double s1 = (V(k+Nv+1) - V(k-Nv-1))/(2.0*ds);
                double s2 = (V(k+Nv+1) - 2.0*V(k) + V(k-Nv-1))/ds/ds;
                double dvcross = (V(k+ Nv + 2) - V(k+ Nv) - V(k-Nv) + V(k-Nv-2))/(4.0*ds*dv);

                rhs_S(i-1) = V(k) + dt/2.0*(bb*s1 + aa*s2 + cross*dvcross);
                // RHS: V_star + theta * dt * (L_S + L_v - r) V_star - theta * dt * (L_S + L_Sv - r) V^{n+1}
//                double Ls_star = -(a * (V_star(k + Nv + 1) - 2 * V_star(k) + V_star(k - Nv - 1)) + 
//                                   d * (V_star(k + Nv + 1) - V_star(k - Nv - 1)));
//                double Lv_star = -(c * (V_star(k + 1) - 2 * V_star(k) + V_star(k - 1)) + 
//                                   e * (V_star(k + 1) - V_star(k - 1)));
//                double Ls_n = -(a * (V(k + Nv + 1) - 2 * V(k) + V(k - Nv - 1)) + 
//                                d * (V(k + Nv + 1) - V(k - Nv - 1)));
//                double Lsv_n = -(b_coeff * (V(k + Nv + 2) - V(k + Nv) - V(k - Nv) + V(k - Nv - 2)));
//                rhs_v(j - 1) = V_star(k) + theta * dt * (Ls_star + Lv_star - r * V_star(k)) - 
//                               theta * dt * (Ls_n + Lsv_n - r * V(k));
            }
